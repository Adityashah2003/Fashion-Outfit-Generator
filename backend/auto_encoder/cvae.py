...
class Sampling(Layer):
    ...
    def compute_output_shape(self, input_shape):
        return input_shape[0]  # Assuming the first input is the mean

def build_cvae_model(input_shape, output_shape, latent_dim):
    input_data = Input(shape=input_shape)
    output_data = Input(shape=output_shape)

    encoder_input = Dense(256, activation='relu', name='encoder_dense_1')(input_data)
    mean = Dense(latent_dim, name='encoder_mean')(encoder_input)
    log_var = Dense(latent_dim, name='encoder_log_var')(encoder_input)
    latent = Sampling(name='encoder_latent')([mean, log_var])

    output_encoder_input = Dense(256, activation='relu', name='output_encoder_dense_1')(output_data)
    output_mean = Dense(latent_dim, name='output_encoder_mean')(output_encoder_input)
    output_log_var = Dense(latent_dim, name='output_encoder_log_var')(output_encoder_input)
    output_latent = Sampling(name='output_latent')([output_mean, output_log_var])

    combined_latent = Concatenate(name='combined_latent')([latent, output_latent])

    hidden_decoder = Dense(256, activation='relu', name='decoder_dense_1')(combined_latent)
    combined_output = Dense(output_shape[0], activation='sigmoid', name='combined_output')(hidden_decoder)
...
def main():
    ...
    saved_model_path = "backend\\auto_encoder\\model\\cvae_model.h5"  
    ...
    saved_model_path = "backend\\auto_encoder\\model\\cvae_model.h5.keras"
    ...
    if not tf.io.gfile.exists(saved_model_path):
        ...
        cvae.fit([input_train_data_split, output_train_data_split], epochs=NUM_EPOCHS, batch_size=BATCH_SIZE,
                validation_data=([input_val_data, output_val_data], None))

        cvae.save(saved_model_path)
    else:
        # cvae = tf.keras.models.load_model(saved_model_path, compile=False, custom_objects={"Lambda": tf.keras.layers.Lambda, "sampling": sampling}, safe_mode=False)
        cvae = tf.keras.models.load_model(saved_model_path, compile=False, custom_objects={"Lambda": tf.keras.layers.Lambda, "sampling": sampling}, safe_mode=False)
